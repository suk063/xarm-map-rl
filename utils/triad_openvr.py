...
# Convert the c matrix to a numpy array
def pose_to_numpy(pose_matrix):
    arr = np.eye(4)
    for i in range(3):
        for j in range(4):
            arr[i,j] = pose_matrix[i][j]
    return arr 
...
class pose_sample_buffer():
    ...
    def append(self,pose_mat,t):
        self.time.append(t)
        self.poses.append(pose_to_numpy(pose_mat))
...
class vr_tracked_device():
    ...
    def get_pose_matrix(self, pose=None):
        if pose == None:
            pose = get_pose(self.vr)
        if pose[self.index].bPoseIsValid:
            return pose_to_numpy(pose[self.index].mDeviceToAbsoluteTracking)
        else:
            return None
